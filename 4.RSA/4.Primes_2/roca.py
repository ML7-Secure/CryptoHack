...
def modular_univariate(f, N, m, t, X):
    ...
    #     yield roots[x],
    L, monomials = create_lattice(pr, shifts, [X], order=None)
    L = reduce_lattice(L)
    polynomials = reconstruct_polynomials(L, f, N ** m, monomials, [X])
    for roots in find_roots(pr, polynomials):
        yield roots[x],
...
# Parameters from fast_primes.py
...
m = 5
t = m+1
print( factorize(N, M, m, t, g=65537) )